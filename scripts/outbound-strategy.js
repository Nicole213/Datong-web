...
let strategyData = [
    {
        ...
        updater: '张三',
        rules: [
            { priority: 1, rule: '组盘时间先进先出', ignoreOnFail: '否' },
    ...
    },
    {
        ...
        materialCode: '-',
        materialName: '-',
        remark: 'A区专用出库策略，优先整托出库',
        createTime: '2024-01-12 09:00:00',
        updateTime: '2024-01-18 16:20:00',
        updater: '李四',
        rules: [
            { priority: 1, rule: '整托优先', ignoreOnFail: '否' },
            { priority: 2, rule: '层升序', ignoreOnFail: '是' }
        ]
    },
    {
        id: 3,
...
function openAddModal() {
    ...
    document.getElementById('modalTitle').textContent = '新增出库策略';
        ...
        <tr>
            ...
            <td>
                <select class="rule-select" onchange="updateRule(${index}, 'rule', this.value)">
                    <option value="组盘时间先进先出" ${rule.rule === '组盘时间先进先出' ? 'selected' : ''}>组盘时间先进先出</option>
                    <option value="组盘时间后进先出" ${rule.rule === '组盘时间后进先出' ? 'selected' : ''}>组盘时间后进先出</option>
                    <option value="整托优先" ${rule.rule === '整托优先' ? 'selected' : ''}>整托优先</option>
...
function editStrategy(id) {
    ...
    document.getElementById('modalTitle').textContent = '编辑出库策略';
